...
### 工具与口径要求（必须遵守）
- **你拥有 `web_search_preview` 工具**，请直接调用它检索网络上的真实内容；
- **禁止**输出任何形如 "我无法联网 / 我无法访问外网 / 我无法实时搜索 / I cannot access the internet / 我无法打开链接 / 我目前无法浏览网页 / 抱歉，作为大语言模型..." 之类的免责声明；
- 如果工具调用失败或没找到内容，**直接**输出"未找到公开讨论"，不要解释模型限制；
- 所有"重要说明 / 我必须先声明 / 在开始之前 / 重要提示" 这类铺垫段落都禁止出现；正文一上来就是结论。
