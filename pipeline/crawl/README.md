...
6 个顺序 stage + 3 条并行旁路（`seed_scholars.py`、`mentions.py`、`xhs_recruitment.py`）。每个 stage 都是**幂等**的（按 DB 标志位跳过已完成的行），所以 `run_all.py` 可以反复 resume —— 没干完的接着干，全干完的直接 skip 进下一阶段。
...
- `xhs_recruitment.py` — 小红书招生情报旁路：按导师抓公开主贴、筛选招生帖、一次 LLM 汇总并写回主库。**不**阻塞 01–06 主线。

`xhs_recruitment.py` 的最小用法：

```bash
cp data/xiaohongshu/config.example.yaml data/xiaohongshu/config.yaml
python crawl/xhs_recruitment.py --check
python crawl/xhs_recruitment.py --advisor-id 50274 --dry-run
python crawl/xhs_recruitment.py --advisor-id 50274
```

它默认读取本地私有配置 `pipeline/data/xiaohongshu/config.yaml`；示例模板是
`config.example.yaml`，不要把真实配置提交进仓库。密钥统一从 `backend/.env`
中的 `TIKHUB_API_KEY` / `ANTHROPIC_API_KEY` 读取；如果你用的是私有
Anthropic 兼容代理，只改本地 `config.yaml` 里的 `llm.base_url`。本地运行产物
写到 `pipeline/data/xiaohongshu/output/YYYY-MM-DD/advisor_<id>/run_<run_id>/`，
正式状态写入 `xhs_crawl_runs`。
...
| 何时调 | 入口函数 | 目的 |
|---|---|---|
| Stage 2 | `crawl_school_colleges` → `extract_college_list` | 从院系列表页启发式抽取 `[{name, url, discipline_category}, ...]` |
| Stage 3 | `crawl_college_advisors` → `extract_advisor_list` | 从学院师资页启发式抽取 `[{name, title, profile_url}, ...]` |
| Stage 3 | `find_faculty_list_link` | 学院主页不一定是师资页，用启发式找"师资/教师"入口链接 |
| Stage 4 | `crawl_advisor_detail` → `_call_llm` | 把单个老师页面 HTML 喂给 LLM，输出 `{bio, research_areas, email, education, honors}` |

Stage 4 的模型由 `backend/.env` 配置：

- 默认复用 OpenAI-compatible 的 `LLM_API_BASE` / `LLM_API_KEY` / `LLM_FALLBACK_MODEL`
- 如果配置 `LLM_CRAWL_PROVIDER=anthropic`，则使用 Anthropic-compatible `/v1/messages` 协议
- `LLM_CRAWL_API_BASE` / `LLM_CRAWL_API_KEY` / `LLM_CRAWL_MODEL` 可以单独覆盖导师详情抽取模型
- `LLM_CRAWL_THINKING=off|low|<token_budget>` 控制 Anthropic-compatible 的 thinking 参数；`off` 表示不主动传 thinking 字段
- `LLM_CRAWL_PROMPT_PROFILE=default|deepseek_strict` 控制导师详情抽取 prompt；`deepseek_strict` 会额外约束 DeepSeek 容易误判的“当前导师身份”和 honors 兼职混入问题
