...
async def main():
    ...
    async with async_session() as db:
        for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            advisor_id = advisor.id if advisor else 0   # 0 = unlinked (sentinel)
            pending_name = "" if advisor else (rec.get("advisor_name") or "")
            pending_school = "" if advisor else (rec.get("school_name") or "")
            if not advisor:
                unlinked_count += 1
                key = f"{rec.get('advisor_name','?')}@{rec.get('school_name','?')}"
                if len(unlinked_examples) < 10:
                    unlinked_examples.append(key)
                log.info("line %d: no advisor in DB → store as unlinked: %s", line_no, key)

            url = (rec.get("url") or "").strip()
            if args.dedup_by_url and url:
                # Dedup by (url, advisor_id OR pending name+school)
                if advisor:
                    ...
                    exists = (await db.execute(
                        select(AdvisorMention.id).where(
                            ...
                            AdvisorMention.pending_advisor_name == pending_name,
                            AdvisorMention.pending_school_name == pending_school,
                            AdvisorMention.url == url,
                        )
                    )).scalar_one_or_none()
                if exists:
    ...
    log.info(
        "Done. inserted=%d (linked=%d, unlinked=%d)  skipped_dup=%d  parse_errors=%d  dry_run=%s",
        inserted, inserted - unlinked_count, unlinked_count,
        skipped_dup, parse_errors, args.dry_run,
    )
    if unlinked_examples:
        log.info("Unlinked examples (kept for reconcile): %s", unlinked_examples)
